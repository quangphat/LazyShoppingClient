...
export class SelectionJobSkill extends React.Component<ISelectionJobSkillProps, ISelectionJobSkillStates> {
    ...
    private onRemove(item: string) {
        ...
        let index = suggested.findIndex(p => p.display == item)
        if (index > -1) {
    ...
    private onSelectSuggested(item: IOptionSimple) {
        this.onClickItem(item)
        let { suggested } = this.state
        let index = suggested.findIndex(p => p.id == item.id)
        if (index > -1) {
    ...
    private renderSuggesteds() {
        ...
        return <React.Fragment>
            {selectedCount < suggested.length ?
                <div>
                <p>Đề xuất cho bạn:</p>
                    <JobSkillTag skills={suggested} onClickItem={(item) => this.onSelectSuggested(item)} />
                </div>
                : null}
        </React.Fragment>
    }
